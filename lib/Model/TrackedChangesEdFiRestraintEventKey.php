...
/**
 * TrackedChangesEdFiRestraintEventKey Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class TrackedChangesEdFiRestraintEventKey implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'trackedChanges_edFi_restraintEventKey';
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['restraint_event_identifier']) && (mb_strlen($this->container['restraint_event_identifier']) > 36)) {
            $invalidProperties[] = "invalid value for 'restraint_event_identifier', the character length must be smaller than or equal to 36.";
        }

        if (!is_null($this->container['restraint_event_identifier']) && (mb_strlen($this->container['restraint_event_identifier']) < 1)) {
     ...
     *
     * @param string|null $restraint_event_identifier A unique number or alphanumeric code assigned to a restraint event by a school, school system, state, or other agency or entity.
     *
     * @return self
     */
    public function setRestraintEventIdentifier($restraint_event_identifier)
    {
        if (!is_null($restraint_event_identifier) && (mb_strlen($restraint_event_identifier) > 36)) {
            throw new \InvalidArgumentException('invalid length for $restraint_event_identifier when calling TrackedChangesEdFiRestraintEventKey., must be smaller than or equal to 36.');
        }
        if (!is_null($restraint_event_identifier) && (mb_strlen($restraint_event_identifier) < 1)) {
            throw new \InvalidArgumentException('invalid length for $restraint_event_identifier when calling TrackedChangesEdFiRestraintEventKey., must be bigger than or equal to 1.');
        }
    ...
    public function setStudentUniqueId($student_unique_id)
    {
        if (!is_null($student_unique_id) && (mb_strlen($student_unique_id) > 32)) {
            throw new \InvalidArgumentException('invalid length for $student_unique_id when calling TrackedChangesEdFiRestraintEventKey., must be smaller than or equal to 32.');
        }
        if (!is_null($student_unique_id) && (mb_strlen($student_unique_id) < 1)) {
            throw new \InvalidArgumentException('invalid length for $student_unique_id when calling TrackedChangesEdFiRestraintEventKey., must be bigger than or equal to 1.');
        }
