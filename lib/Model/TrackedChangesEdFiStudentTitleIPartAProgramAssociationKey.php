...
/**
 * TrackedChangesEdFiStudentTitleIPartAProgramAssociationKey Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class TrackedChangesEdFiStudentTitleIPartAProgramAssociationKey implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'trackedChanges_edFi_studentTitleIPartAProgramAssociationKey';
    ...
    public function setProgramName($program_name)
    {
        if (!is_null($program_name) && (mb_strlen($program_name) > 60)) {
            throw new \InvalidArgumentException('invalid length for $program_name when calling TrackedChangesEdFiStudentTitleIPartAProgramAssociationKey., must be smaller than or equal to 60.');
        }
        if (!is_null($program_name) && (mb_strlen($program_name) < 1)) {
            throw new \InvalidArgumentException('invalid length for $program_name when calling TrackedChangesEdFiStudentTitleIPartAProgramAssociationKey., must be bigger than or equal to 1.');
        }
    ...
    public function setProgramTypeDescriptor($program_type_descriptor)
    {
        if (!is_null($program_type_descriptor) && (mb_strlen($program_type_descriptor) > 306)) {
            throw new \InvalidArgumentException('invalid length for $program_type_descriptor when calling TrackedChangesEdFiStudentTitleIPartAProgramAssociationKey., must be smaller than or equal to 306.');
        }
    ...
    public function setStudentUniqueId($student_unique_id)
    {
        if (!is_null($student_unique_id) && (mb_strlen($student_unique_id) > 32)) {
            throw new \InvalidArgumentException('invalid length for $student_unique_id when calling TrackedChangesEdFiStudentTitleIPartAProgramAssociationKey., must be smaller than or equal to 32.');
        }
        if (!is_null($student_unique_id) && (mb_strlen($student_unique_id) < 1)) {
            throw new \InvalidArgumentException('invalid length for $student_unique_id when calling TrackedChangesEdFiStudentTitleIPartAProgramAssociationKey., must be bigger than or equal to 1.');
        }
