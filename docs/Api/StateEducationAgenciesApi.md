...
```php
getStateEducationAgencies($offset, $limit, $min_change_version, $max_change_version, $total_count, $state_education_agency_id, $id, $use_snapshot): \Resources\Model\EdFiStateEducationAgency[]
```
...
try {
    $result = $apiInstance->getStateEducationAgencies($offset, $limit, $min_change_version, $max_change_version, $total_count, $state_education_agency_id, $id, $use_snapshot);
    print_r($result);
} catch (Exception $e) {
